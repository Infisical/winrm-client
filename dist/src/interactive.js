...
exports.monitorCommandOutput = monitorCommandOutput;
exports.detectPromptPattern = detectPromptPattern;
exports.pollCommandWithTimeout = pollCommandWithTimeout;
const command_1 = require("./command");
const logger_1 = require("./utils/logger");
const logger = (0, logger_1.createLogger)('interactive');
/**
 * Monitor the output of an interactive command, detect prompts, and send responses as needed.
 */
async function monitorCommandOutput(params) {
...
async function detectPromptPattern(output, prompts) {
    if (!output)
        return null;
            ...
            if (prompt.pattern) {
                try {
                    ...
                    logger.debug('Pattern fallback also failed', {
                        error: patternError instanceof Error
...
async function* pollCommandWithTimeout(params, executionTimeout, pollInterval) {
    const startTime = Date.now();
    ...
    while (Date.now() - startTime < executionTimeout) {
        try {
            ...
            const result = await Promise.race([
                (0, command_1.doReceiveOutputNonBlocking)(params),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Polling timed out after ${executionTimeout}ms`)), remaining)),
            ]);
            yield result;
            if (result.isComplete) {
                return;
