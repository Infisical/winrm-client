...
function createNTLMHash(password) {
    const hash = js_md4_1.default.create();
    hash.update(Buffer.from(password, 'ucs2'));
    return Buffer.from(hash.arrayBuffer());
}
function createNTLMv2Hash(ntlmHash, username, domain) {
 ...
 */
function createType1Message(workstation, domain) {
    ...
    const flags = NTLMFLAG_NEGOTIATE_OEM |
...
function decodeType2Message(str) {
    ...
    const encoding = flags & NTLMFLAG_NEGOTIATE_UNICODE ? 'ucs2' : 'ascii';
    ...
    const targetName = targetNameLen > 0
        ? buf.toString(encoding, targetNameOffset, targetNameOffset + targetNameLen)
        : '';
...
// ── Type 3 (Authenticate) ──────────────────────────────────────────
function createLMv2Response(type2Message, username, ntlmHash, nonce, domain) {
    const ntlm2Hash = createNTLMv2Hash(ntlmHash, username, domain || type2Message.targetName);
...
function createNTLMv2Response(type2Message, username, ntlmHash, nonce, domain) {
 ...
 */
function createType3Message(type2Message, username, password, workstation, target, type1Bytes, type2Bytes) {
    ...
    const hashTarget = type2Message.targetInfo?.parsed['DOMAIN'] ||
    ...
    const lmv2 = createLMv2Response(type2Message, username, ntlmHash, nonce, actualTarget);
    const ntlmv2 = createNTLMv2Response(type2Message, username, ntlmHash, nonce, actualTarget);
